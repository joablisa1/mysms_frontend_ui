...
<div v-if="currentUser">
    <div class="flex justify-between px-4 mt-4 sm:px-8">
        <h2 class="text-2xl text-gray-600">#ShortCode</h2>
        ...
        <div class="p-4 bg-white rounded">
            ...
            <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                ...
                <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                    <router-link to="/shortcode/add" type="button" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
            ...
            <table class="w-full mt-2 text-gray-500 ">
                ...
                <tbody v-if="shortcodes.length > 0">
                    <tr v-for="(shortcode,key) in shortcodes" :key="key" class="border-b dark:border-gray-700">
                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <input v-model="selectAll" type="checkbox" class="w-5 h-5 text-blue-500 border-gray-300 rounded cursor-pointer focus:ring-0" />
                        </th>
                        <td class="px-4 py-3">{{ shortcode.shortcode }}</td>
                        <td class="px-4 py-3 lowercase">{{ shortcode.category }}</td>
                        <td class="px-4 py-3 lowercase">{{ shortcode.telco ? shortcode.telco : 'Empty' }}</td>
                        <td class="px-4 py-3">{{ shortcode.organization.organizationName }}</td>
                        <td class="px-4 py-3 lowercase">{{ shortcode.category }}</td>
                        <td class="px-4 py-3"></td>                 
                        <td class="flex items-center justify-center px-4 py-3 space-x-3">
                            <router-link   :to="'/shortcode/edit/'+ shortcode.id" href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
    ...
    c2.518,2.51,3.905,5.855,3.905,9.414C470.516,51.036,469.127,54.38,466.61,56.897z" />
                            ...
                            <button @click.prevent=" deleteShortCodeById(shortcode.id)" href="#" class="font-medium text-red-600 dark:text-red-500 hover:underline">
    ...
    methods: {
        ...
        async retrieveShrotcodes() {
            ...
            try {
                ...
                ShortcodeService.getAll(params)
                    .then(response => {
        ...
        deleteShortCodeById(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ShortcodeService.deleteShortCode(id).then(() => {
                        this.refreshList();
                    })
        ...
        // },
        searchName() {
            ShortcodeService.findByTitle(this.title)
                .then(response => {
