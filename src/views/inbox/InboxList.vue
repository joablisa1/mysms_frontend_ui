<template>
    <div v-if="currentUser">
        <div class="flex justify-between px-4 mt-4 sm:px-8">
            <h2 class="text-2xl text-gray-600">#Inbox Messages</h2>
            ...
            <div class="p-4 bg-white rounded">
                ...
                <table class="w-full mt-2 text-gray-500">
                    ...
                    <tbody v-if="organizations.length > 0">
                        <tr v-for=" inbox in  inboxes" :key=" inbox.id" class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <input v-model="selectAll" type="checkbox" class="w-5 h-5 text-blue-500 border-gray-300 rounded cursor-pointer focus:ring-0" />
                            </th>
                            <td class="px-4 py-3">{{ inbox.msisdn }}</td>
                            <td class="px-4 py-3">{{ inbox.shortCode.shortcode }}</td>
                            <td class="px-4 py-3">{{ inbox.keyword }}</td>
                            <td class="px-4 py-3">{{ inbox.content }}</td>
                            <td class="px-4 py-3">{{ inbox.status  }}</td>
    ...
    methods: {
        ...
        async retrieveInboxes() {
            ...
            try {
                ...
                InboxService.getAllInbox(params)
                    .then(response => {
                        this.inboxes = response.data.items;
