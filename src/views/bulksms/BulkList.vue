...
<div v-if="currentUser">
    <div class="flex justify-between px-4 mt-4 sm:px-8">
        <h2 class="text-2xl text-gray-600">Bulk Messages</h2>
        ...
        <div class="flex items-center space-x-1 text-xs">
            ...
            <span class="text-gray-600">bulk</span>
        </div>
    </div>
            ...
            <div class="bg-white dark:bg-gray-800">
                <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                    ...
                    <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                        <router-link to="/bulk/send" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                ...
                <table class="w-full mt-2 text-gray-500 dark:text-gray-400">
                    ...
                    <tbody v-if="bulksmses.length > 0">
                        <tr v-for="(bulksms,key) in bulksmses" :key="key" class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <input v-model="selectAll" type="checkbox" class="w-5 h-5 text-blue-500 border-gray-300 rounded cursor-pointer focus:ring-0" />
                            </th>
                            <td class="px-4 py-3">{{ bulksms.bulkName }}</td>
                            <td class="px-4 py-3">{{ bulksms.shortCode.shortcode }}</td>
                            <td class="px-4 py-3">{{ bulksms.fileUuId }}</td>
                            <td class="px-4 py-3">{{ bulksms.oa }}</td>
                            <td class="px-4 py-3">{{ bulksms.message }}</td>
                            <td class="px-4 py-3">{{ bulksms.matchingId}}</td>
                            <td class="px-4 py-3">{{ bulksms.bulkStatus}}</td>
                            <td class="px-4 py-3">
                                <!-- add a link to a view component for the bulksms.bulkName property -->
                                <router-link :to="{ name: 'BulkItem', params: { id: bulksms.matchingId }, query: { q: bulksms.matchingId } }">
                                    {{ bulksms.matchingId  }}
                                </router-link>
                            </td>
                            <td class="px-4 py-3">{{ bulksms.actionResponseUrl }}</td>
                            <td class="px-4 py-3">{{ bulksms.creationDate  }}</td>
    ...
    methods: {
        ...
        async retrieveBulkSms() {
            ...
            try {
                ...
                BulksmsService.getAll(params)
                    .then(response => {
                        this.bulksmses = response.data.items;
                        this.currentPage = response.data.currentPage;
                        this.totalItems = response.data.totalItems;
                        this.totalPages = response.data.totalPages;
                        console.log(this.bulksmses.items);
                    })
