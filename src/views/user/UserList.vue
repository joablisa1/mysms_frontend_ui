...
<div v-if="currentUser">
    <div class="flex justify-between px-4 mt-4 sm:px-8">
        <h2 class="text-2xl text-gray-600">User Management</h2>
    ...
    <div class="p-4 mt-8 sm:px-8 sm:py-4">
        <div class="p-4 bg-white rounded">
            ...
            <div class="flex flex-col items-center justify-between p-4 space-y-3 lg:flex sm:flex-col md:flex-row md:space-y-0 md:space-x-4">
                ...
                <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                    <router-link to="/user/add"  class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
            ...
            <table class="w-full mt-2 text-gray-500">
                ...
                <tbody>
                    <tr v-for="(user, key) in users" :key="key" class="border-b dark:border-gray-700">
                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <input v-model="selectAll" type="checkbox" class="w-5 h-5 text-blue-500 border-gray-300 rounded cursor-pointer focus:ring-0" />
                        </th>
                        <td class="px-4 py-3">{{ user.username }}</td>
                        <td class="px-4 py-3">{{ user.organization.organizationName }}</td>
                        <td class="px-4 py-3">{{ user.email }}</td>
                        <td class="px-3 py-3">
                            {{
                  user.roles
                    .map((role) => {
                      const cleanedRole = role.name
                        .replace("ROLE_", "")
                        .replace("_", " ");
                      return (
                        cleanedRole.charAt(0).toUpperCase() +
                        cleanedRole.slice(1).toLowerCase()
                      );
                    })
                    .join(", ")
                }}
                        </td>
                        <td class="px-4 py-3">{{ user.active ? "Active" : "Inactive" }}</td>
                        <td class="flex items-center px-4 py-3 space-x-3">
                            <button href="#"  @click.prevent="$router.push('/user/editUser/' + user.id)"  class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
    ...
    c2.518,2.51,3.905,5.855,3.905,9.414C470.516,51.036,469.127,54.38,466.61,56.897z" />
                            ...
                            <button href="#" v-on:click="deleteUserById(user.id)" class="font-medium text-red-600 dark:text-red-500 hover:underline">
...
export default {
    ...
    methods: {
        ...
        async retrieveUsers() {
            ...
            try {
                if (!hasRootPrivilege) {
                    params['organization'] = organizationId; //Tech debt if user does not exist in any organization then return anauthorized page/route
                }
